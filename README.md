...
![Version](https://img.shields.io/badge/version-v1.0.0-2563eb.svg)
![Platform](https://img.shields.io/badge/platform-Web%20%7C%20Desktop-0ea5e9.svg)
![Test](https://img.shields.io/badge/test-54%2F54-22c55e.svg)

> 本地优先的 PDF 编辑器 —— 编辑、转换、一站式完成。文件全程在本机处理，零上传、零留存。

![Pdfly 首页](./docs/screenshots/home.png)

**Pdfly** 是一个 Web + 桌面（Electron）双端的完整 PDF 工具，覆盖在线编辑、PDF 转图片、图片转 PDF 三大核心场景。所有解析、渲染与转换均通过本地库完成，不依赖任何服务器，可完全离线使用。
...
| 能力 | 说明 |
| --- | --- |
| 文本编辑 | 点击文本原位替换，导出保留未编辑部分的可搜索文本 |
| 批注 / 高亮 | 多色批注与半透明高亮标记 |
| 元素插入 | 图片、矩形 / 椭圆 / 箭头 / 线条、手写签名 |
| 页面管理 | 插入空白页、删除页面、缩略图拖拽重排 |
| 历史记录 | 撤销 / 重做（≥20 步），实时所见即所得预览 |
...
| --- | --- |
| 输出格式 | PNG（支持透明背景）、JPG |
| 输出控制 | 自定义 DPI（72–600）、目标分辨率与尺寸 |
| 页范围 | 单页、全部页面或 `1-5,7` 混合范围 |
| 任务控制 | 进度显示、随时取消、失败页单独重试 |
| 下载 | 单张下载或 ZIP 批量打包 |
...
| 能力 | 说明 |
| --- | --- |
| 输入格式 | PNG / JPG / GIF / BMP / WEBP |
| 多图合并 | 单 PDF 多页，拖拽排序、旋转、删除 |
| 版式控制 | 页面尺寸（A4 / Letter / 自定义）、页边距、缩放比例 |
| 容错 | 损坏图片自动隔离标记，不影响其他图片 |

### 隐私与安全

- 所有处理均在本地完成，文件零上传、零留存
- 依赖与资源本地打包，无运行时 CDN 请求
- 桌面端可完全断网使用
...
| 平台 | 浏览器 / 运行时 | 说明 |
| --- | --- | --- |
| Web | Chrome / Edge / Firefox / Safari（近两个大版本） | 依赖本地打包，无 CDN |
| 桌面 | Windows 10+（nsis / portable 安装包） | macOS / Linux 可自行构建 |
| 构建 | Node v22+、npm 10+ | 中文字体需在构建期下载一次 |
...
| 场景 | 行为 |
| --- | --- |
| 大文件 | Web 端超过 100MB 提示内存风险但允许尝试；桌面端更从容 |
| 多页文档 | 缩略图 >50 页自动懒加载（IntersectionObserver） |
| 渲染 | 画布仅渲染当前页 ±1，滚动切换即时 |
| 图片输出上限 | 单边 16384px / 面积 2.2 亿像素，超限提示降低 DPI 或倍率 |
| GIF 输入 | 仅取第一帧（静态帧），动画序列不在本期范围 |
...
**文件会被上传到服务器吗？**
不会。所有解析、渲染、编辑与转换均在本地完成，无任何网络请求（字体下载除外，仅构建期）。

**桌面端可以离线使用吗？**
可以。全部依赖与资源本地打包，断网状态下所有功能可用。

**支持哪些图片格式？**
PNG / JPG / GIF / BMP / WEBP。其中 GIF 取第一帧静态画面。

**导出后 PDF 的文本还能搜索吗？**
未编辑部分的原文内容流完整保留，可正常搜索、选中、复制；被替换的文本区域按你的编辑结果呈现。

**中文导出为什么受限？**
导出中文需要嵌入 Noto Sans SC 字体。首次安装会自动下载（`npm run fetch:font`）；网络不可用时自动降级 Helvetica，仅中文文本受影响。

**我的数据会留下痕迹吗？**
会话内数据全部保存在内存中，关闭页面或应用即清理，不留任何本地缓存文件。

---

## 路线图

- [ ] PWA 离线支持（Web 端可安装、离线打开）
- [ ] 透明背景 PNG 导出优化
- [ ] 图片压缩选项（导出体积控制）
- [ ] 国际化（i18n）
- [ ] 桌面端文件关联与右键打开
